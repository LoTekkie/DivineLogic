...
REM Set the path to the Papyrus Compiler
...
REM Set the output directory for compiled scripts in Skyrim's Data folder
SET output="C:\Program Files (x86)\Steam\steamapps\common\Skyrim Special Edition\Data\Scripts"

REM Copy all modified source files to the Skyrim Scripts folder
XCOPY %compile%\* %source% /Y

REM Compile the scripts using the Papyrus Compiler
%compiler% %compile% -all -f=%source%\TESV_Papyrus_Flags.flg -i=%source% -o=%output%

REM Pause the script so the user can see the output before closing
PAUSE
